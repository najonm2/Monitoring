...
if last_8_runs:
    ...
    for i, run in enumerate(last_8_runs, 1):
        ...
        print(f"   :{succeeded:2d} :{running:2d} :{failed:2d} :{suspended:2d}")
...
if current_jobs:
    ...
    if len(unique_assets) < len(current_jobs):
        print(f"  Warning: {len(current_jobs)} jobs but only {len(unique_assets)} unique assets (duplicates present)")
